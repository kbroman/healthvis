...
var HealthvisManyBox;

HealthvisManyBox = function() {
  this.width = 1000;
  this.height = 450;
  this.init = function(elementID, d3Params) {
    ...
    this.svg = d3.select('#main').append('svg').attr('width', this.width).attr('height', this.height).attr('class', 'chart');
    return this.lowsvg = d3.select('#main').append('svg').attr('width', this.width).attr('height', this.height);
  };
  this.visualize = function() {
    var Baxis, BaxisData, Laxis, LaxisData, botylim, br2, breaks, clickStatus, colindex, counts, curves, d, fix4hist, formatAxis, grp4BkgdHist, h, hi, hist, histColors, histline, i, ind, indRect, indRectGrp, indindex, j, lo, longRect, longRectGrp, lowBaxis, lowBaxisData, lowxScale, lowyScale, midQuant, nQuant, pad, qu, quant, qucolors, quline, randomInd, recWidth, rightAxis, tmp, topylim, w, x, xScale, xlab, yScale, ylab, _i, _j, _k, _l, _len, _len1, _len2, _len3, _len4, _len5, _m, _n, _o, _ref, _ref1, _ref2;
    ylab = this.ylab;
    xlab = this.xlab;
    ind = this.ind;
    qu = this.qu;
    ...
    br2 = [];
    for (_k = 0, _len2 = breaks.length; _k < _len2; _k++) {
      i = breaks[_k];
      br2.push(i);
      br2.push(i);
    }
    ...
    return this.svg.append("text").text(xlab).attr("x", (w - pad.left - pad.bottom) / 2 + pad.left).attr("y", h - pad.bottom * 0.2).attr("fill", "blue").attr("dominant-baseline", "middle").attr("text-anchor", "middle");
  };
